...
def findContours(ilastik_cells,cont_find_it=2):
	"""helper function which finds contours in a segmented image
	...
	-------
	list of cv2 contours
		all contours/initial clusters that were found
	"""
	
	ilastik_cells = cv2.dilate(ilastik_cells, np.ones((3,3),np.uint8),iterations=cont_find_it)
	ilastik_cells = np.invert(ilastik_cells)
	im2,Cont,hierarchy = cv2.findContours(ilastik_cells,mode=cv2.RETR_TREE,method=cv2.CHAIN_APPROX_SIMPLE)
...
def filterContours(Cont,ilastik_cells,cont_find_it=2)
	"""helper function which filters contours
	   noise (small blobs predicted to be boundary) from the segmented image is filtered out
	
	Parameters
	----------
	Cont : list of cv2 contours
		all contours/initial clusters
	ilastik_cells : 2-dimensional array of type np.uint8
		segmented image (ilastik segmentation)
	cont_find_it : int (default 2)
		number for the dilations applied to each initial cluster
	...
	-------
	list of cv2 contours
		filtered initial clusters
	"""
	keep_list=[]; dontkeep_list=[]
	
	for i in range(len(Cont)):
		emp = draw_contour(Cont[i],ilastik_cells.shape)
		ar = cv2.contourArea(Cont[i])
		if ar<500:
			dontkeep_list.append(i)
		elif ar>5000:
			keep_list.append(i)
		else:
			emp2 = cv2.erode(emp,np.ones((3,3),np.uint8),iterations=cont_find_it)
			bgcheck = ilastikpic[emp2==255]
			#if more than 10% of the contour are border pixels, this contour is noise
			if (np.sum(bgcheck==255)/float(len(bgcheck)) > 0.1):
				dontkeep_list.append(i)
			else:
				keep_list.append(i)
	
	for el in keep_list:
		if hierarchy[0][el,2]>-1:
			children=np.where(hierarchy[0][:,3]==el)[0]	
			if not all(np.isin(children,dontkeep_list)):
				#bigger contours containing multiple smaller non-noise contours are deleted
				dontkeep_list.append(el)
...
def draw_contour(contour,img_shape, binary=False):
	...
	if binary:
		...
		cv2.drawContours(empty_img, [contour], 0, 1, -1)
	else:
		empty_img = np.zeros(img_shape,dtype=np.uint8)
		cv2.drawContours(empty_img, [contour], 0, 255, -1)
	return empty_img
